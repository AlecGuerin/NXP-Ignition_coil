...
/**
 * @brief Wait at least for the provided micro-seconds.
 * @param delay_us Micro-second to wait for.
 */
__attribute__((weak)) void CLOCK_Delay(uint32_t delay_us){
	assert(0U != delay_us);

	uint32_t count = (uint32_t)USEC_TO_COUNT(delay_us, CLOCK_GetMainClkFreq());
	count = (count / 4U);
	for (; count > 0UL; count--)
	    {
	        __NOP();
